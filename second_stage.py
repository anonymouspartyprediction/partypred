...
if "GAT" in config_dict:
    ...
    USE_GAT = False
if "INCLUDE_MATCHED_VOTERS" in config_dict:
    INCLUDE_MATCHED_VOTERS = config_dict["INCLUDE_MATCHED_VOTERS"]
else:
    INCLUDE_MATCHED_VOTERS = False
...
for list_index, graphs_to_use in enumerate(graphs_to_use_list):
    ...
    results_dict_f1[list_index] = {}
    test_f1_list_normal_normal = []
    test_f1_list_normal_pol = []
    test_f1_list_pol_normal = []
    test_f1_list_pol_pol = []
    test_f1_list_normalpol_normal = []
    test_f1_list_normalpol_pol = []
    ...
    for repeat_num in range(10):
        ...
        if 'text' in graphs_to_use:
            ...
            if len(graphs_to_use) == 1:
                with open(f"{config_dict['GRAPH_FOLDER']}/test_ids.pkl", 'rb') as f:
                    ...
                    key_list = key_list.intersection((list(id_to_weak.keys()) + list(id_to_true.keys())))
        ...
        test_f1_list_normal_normal.append(test_f1)
        ...
        if INCLUDE_POLITICIANS:
            ...
            test_f1 = f1_score(y_test, y_pred_test, average='weighted')
            test_f1_list_normal_pol.append(test_f1)
            ...
            test_f1 = f1_score(y_test, y_pred_test, average='weighted')
            test_f1_list_pol_normal.append(test_f1)
            ...
            test_f1 = f1_score(y_test, y_pred_test, average='weighted')
            test_f1_list_pol_pol.append(test_f1)
            ...
            test_f1 = f1_score(y_test, y_pred_test, average='weighted')
            test_f1_list_normalpol_normal.append(test_f1)
            ...
            test_f1 = f1_score(y_test, y_pred_test, average='weighted')
            test_f1_list_normalpol_pol.append(test_f1)
    ...
    results_dict_f1[list_index]['normal_normal'] = test_f1_list_normal_normal
    if INCLUDE_POLITICIANS:
        results_dict_f1[list_index]['normal_pol'] = test_f1_list_normal_pol
        results_dict_f1[list_index]['pol_normal'] = test_f1_list_pol_normal
        results_dict_f1[list_index]['pol_pol'] = test_f1_list_pol_pol
        results_dict_f1[list_index]['normalpol_normal'] = test_f1_list_normalpol_normal
        results_dict_f1[list_index]['normalpol_pol'] = test_f1_list_normalpol_pol

results_df = pd.DataFrame()

for key in results_dict.keys():
    print(graphs_to_use_list[key])
    results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'normal', 'test_data':'normal', 'mean':round(100*np.mean(results_dict[key]['normal_normal']),1), 'std':round(100*np.std(results_dict[key]['normal_normal']),1)}, index=[0])])
    ...
    if INCLUDE_POLITICIANS:
        results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'normal', 'test_data':'politician', 'mean':round(100*np.mean(results_dict[key]['normal_pol']),1), 'std':round(100*np.std(results_dict[key]['normal_pol']),1)}, index=[0])])
        results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'politician', 'test_data':'normal', 'mean':round(100*np.mean(results_dict[key]['pol_normal']),1), 'std':round(100*np.std(results_dict[key]['pol_normal']),1)}, index=[0])])
        results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'politician', 'test_data':'politician', 'mean':round(100*np.mean(results_dict[key]['pol_pol']),1), 'std':round(100*np.std(results_dict[key]['pol_pol']),1)}, index=[0])])
        results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'normalpol', 'test_data':'normal', 'mean':round(100*np.mean(results_dict[key]['normalpol_normal']),1), 'std':round(100*np.std(results_dict[key]['normalpol_normal']),1)}, index=[0])])
        results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'normalpol', 'test_data':'politician', 'mean':round(100*np.mean(results_dict[key]['normalpol_pol']),1), 'std':round(100*np.std(results_dict[key]['normalpol_pol']),1)}, index=[0])])
...
for key in results_dict.keys():
    print(graphs_to_use_list[key])
    results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'normal', 'test_data':'normal', 'mean':round(100*np.mean(results_dict_f1[key]['normal_normal']),1), 'std':round(100*np.std(results_dict_f1[key]['normal_normal']),1)}, index=[0])])
    print('AVERAGE TEST F1, normal -> normal', round(100*np.mean(results_dict_f1[key]['normal_normal']),1), '+-', round(100*np.std(results_dict_f1[key]['normal_normal']),1))
    if INCLUDE_POLITICIANS:
        results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'normal', 'test_data':'politician', 'mean':round(100*np.mean(results_dict_f1[key]['normal_pol']),1), 'std':round(100*np.std(results_dict_f1[key]['normal_pol']),1)}, index=[0])])
        results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'politician', 'test_data':'normal', 'mean':round(100*np.mean(results_dict_f1[key]['pol_normal']),1), 'std':round(100*np.std(results_dict_f1[key]['pol_normal']),1)}, index=[0])])
        results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'politician', 'test_data':'politician', 'mean':round(100*np.mean(results_dict_f1[key]['pol_pol']),1), 'std':round(100*np.std(results_dict_f1[key]['pol_pol']),1)}, index=[0])])
        results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'normalpol', 'test_data':'normal', 'mean':round(100*np.mean(results_dict_f1[key]['normalpol_normal']),1), 'std':round(100*np.std(results_dict_f1[key]['normalpol_normal']),1)}, index=[0])])
        results_df = pd.concat([results_df, pd.DataFrame({'data_types':'+'.join(graphs_to_use_list[key]), 'train_data':'normalpol', 'test_data':'politician', 'mean':round(100*np.mean(results_dict_f1[key]['normalpol_pol']),1), 'std':round(100*np.std(results_dict_f1[key]['normalpol_pol']),1)}, index=[0])])
        print('AVERAGE TEST ACC, normal -> politician', round(100*np.mean(results_dict_f1[key]['normal_pol']),1), '+-', round(100*np.std(results_dict_f1[key]['normal_pol']),1))
        print('AVERAGE TEST ACC, politician -> normal', round(100*np.mean(results_dict_f1[key]['pol_normal']),1), '+-', round(100*np.std(results_dict_f1[key]['pol_normal']),1))
        print('AVERAGE TEST ACC, politician -> politician', round(100*np.mean(results_dict_f1[key]['pol_pol']),1), '+-', round(100*np.std(results_dict_f1[key]['pol_pol']),1))
        print('AVERAGE TEST ACC, normal + politician -> normal', round(100*np.mean(results_dict_f1[key]['normalpol_normal']),1), '+-', round(100*np.std(results_dict_f1[key]['normalpol_normal']),1))
        print('AVERAGE TEST ACC, normal + politician -> politician', round(100*np.mean(results_dict_f1[key]['normalpol_pol']),1), '+-', round(100*np.std(results_dict_f1[key]['normalpol_pol']),1))
partial_path_to_save = CONFIG_PATH.split('/')[-1].replace('json','csv')
results_df.to_csv(f'results/f1_weighted_{partial_path_to_save}', index=False)
